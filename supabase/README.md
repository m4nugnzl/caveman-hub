...
| `0027_readonly_when_expired.sql` | Con la `0019`, si vas a cobrar | Una prueba caducada solo impide dar de alta clientes: con tres, se puede usar la aplicación indefinidamente sin pagar. **Cambia quién puede escribir**; leer, exportar y borrar no se tocan. Requiere `0006` y `0019`. |
...
| `0061_escalera_de_planes.sql` | Con la tarifa nueva (`docs/monetizacion.md` §7) | Se siguen vendiendo los dos planes de siempre. **Aplicarla no cambia nada de lo que se ve**: Solo y Equipo pasan a significar otra cosa y nace Pro, pero los tres llegan sin poder contratarse porque todavía no existen en Stripe. Quien los enciende es el bloque «DESPUÉS DE STRIPE» de su propio final. **Deja una tarea a mano y urgente**: mover el `metadata.plan` en Stripe de quien ya paga, o su siguiente renovación le cambia el plan. Se aplica una sola vez. Requiere `0019`, `0021` y **`0025`** — sin esta última falla entera al arrancar, con guarda propia desde que pasó. |
| `0025_billing_product_ref.sql` | **Sí, si cobras** | Un `ALTER TABLE` y dos comentarios, pero **`billing-checkout` lee esa columna**: si falta, el `select` de la función devuelve error y la pasarela contesta «ese plan no se puede contratar» a todo el mundo. Aditiva y sin riesgo. |
...
| `0097_la_semana_nueva_hereda_los_ids_del_plan.sql` | **Sí, cuanto antes si has migrado algún plan al bloque** | Es la `0085` otra vez, por una puerta nueva. Desde que el plan vive en el bloque (`0086`), la pantalla del cliente lee las hojas del bloque —con los ids del bloque— y `log_session_set` sigue buscando el ejercicio en `microcycles[].days[]`. **Toda semana añadida después de migrar el plan nace imposible de registrar**: el primer peso que anota el cliente vuelve con «El ejercicio ex_… no está programado en Lower A», el número se ve en pantalla y no está al recargar, y el rechazo es `P0001`, o sea que ni se reintenta ni se apunta. Reemplaza `continue_program(uuid, jsonb)` con la MISMA firma: no hay que coordinar con el despliegue, y arregla también a las pestañas antiguas porque deja de adoptar los ids que le propongan cuando el bloque tiene plan. No toca tablas, políticas ni datos. **Las semanas que ya nacieron rotas son datos, no código**: se reparan con `npm run reparar:ids`. Requiere `0086` y `0087`. |
